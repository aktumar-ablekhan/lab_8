...
def draw_grid():
    ...
    for i in range(HEIGHT // CELL):
        for j in range(WIDTH // CELL):
            pygame.draw.rect(screen, colors[(i + j) % 2], (i * CELL, j * CELL, CELL, CELL)) #Coordinate coordinate width height
...
class Food:
    ...
    def generate_random_pos(self, snake):
        ...
        while True:
            ...
            if all(segment.x != x or segment.y != y for segment in snake.body): # жылан мен алма кездеспейди
                self.pos = Point(x, y)
                break
